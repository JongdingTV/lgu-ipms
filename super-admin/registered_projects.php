...
if (isset($db) && !$db->connect_error) {
    $res = $db->query("SELECT id, code, name, type, sector, location, status, budget, created_at FROM projects ORDER BY created_at DESC");
    if ($res) {
...
<head>
    ...
    <title>Super Admin Registered Projects - IPMS</title>
...
<body class="super-admin-theme">
    ...
    <header class="nav" id="navbar">
        ...
        <div class="nav-links">
            ...
            <a href="/super-admin/registered_projects.php" class="active"><img src="../assets/images/admin/list.png" class="nav-icon">Registered Projects</a>
    ...
    <section class="main-content">
        <div class="dash-header"><h1>Registered Projects</h1><p>Super Admin registry view for all projects.</p></div>
        <div class="recent-projects">
            <h3>Project List</h3>
            <div class="table-wrap">
                <table class="feedback-table">
                    <thead><tr><th>Code</th><th>Name</th><th>Type</th><th>Sector</th><th>Location</th><th>Status</th><th>Budget</th></tr></thead>
                    <tbody>
                    <?php if (empty($projects)): ?>
                        <tr><td colspan="7">No projects registered yet.</td></tr>
                    <?php else: foreach ($projects as $p): ?>
                        <tr>
                            <td><?php echo htmlspecialchars((string)($p['code'] ?? '-')); ?></td>
                            <td><?php echo htmlspecialchars((string)($p['name'] ?? '-')); ?></td>
                            <td><?php echo htmlspecialchars((string)($p['type'] ?? '-')); ?></td>
                            <td><?php echo htmlspecialchars((string)($p['sector'] ?? '-')); ?></td>
                            <td><?php echo htmlspecialchars((string)($p['location'] ?? '-')); ?></td>
                            <td><?php echo htmlspecialchars((string)($p['status'] ?? '-')); ?></td>
                            <td>PHP <?php echo number_format((float)($p['budget'] ?? 0), 2); ?></td>
                        </tr>
