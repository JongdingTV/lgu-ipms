...
function dept_head_employee_has_column(mysqli $db, string $column): bool
{
    $stmt = $db->prepare(
        "SELECT 1
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    } elseif (is_rate_limited('department_head_login', 6, 300)) {
        $error = 'Too many login attempts. Please try again in a few minutes.';
    } else {
        $hasStatus = dept_head_employee_has_column($db, 'account_status');
        $select = 'id, first_name, last_name, role, password' . ($hasStatus ? ', account_status' : '');
        $stmt = $db->prepare("SELECT {$select} FROM employees WHERE email = ? LIMIT 1");
        if ($stmt) {
            ...
            if (!$employee || !$validPassword) {
                record_attempt('department_head_login');
                $error = 'Invalid email or password.';
            } else {
                if ($hasStatus && strtolower((string)($employee['account_status'] ?? 'active')) !== 'active') {
                    $error = 'Your account is currently inactive. Please contact Super Admin.';
                } else {
                $userRole = strtolower(trim((string) ($employee['role'] ?? '')));
                if (!in_array($userRole, ['department_head', 'department_admin', 'admin', 'super_admin'], true)) {
                    log_security_event('ROLE_DENIED', 'Department Head login blocked for non-authorized role');
                    $error = 'Your account is not assigned to department head access.';
                } else {
...
<head>
    ...
    <title>Department Head Login - LGU IPMS</title>
...
<header class="nav">
    <div class="nav-logo"><img src="../assets/images/icons/ipms-icon.png" alt="LGU Logo"> LGU Department Head Portal</div>
    <a href="/public/index.php" class="home-btn">Home</a>
</header>
<div class="wrapper">
    <div class="card">
        <img src="../assets/images/icons/ipms-icon.png" class="icon-top" alt="LGU">
        <h2 class="title">Department Head Login</h2>
        <p class="subtitle">Approve projects before admin assignment.</p>
