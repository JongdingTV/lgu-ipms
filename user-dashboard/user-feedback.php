        ...
        <div class="card" style="margin-bottom:18px;">
            ...
            <form id="userFeedbackForm" class="user-feedback-form" method="post" action="user-feedback.php" enctype="multipart/form-data">
                ...
                <div class="user-feedback-form-grid">
                    ...
                    <div class="full">
                        <label>Google GPS Pin (Sample - User Side Only)</label>
                        <button type="button" id="gpsPinBtn" class="ac-f84d9680" style="margin-bottom:10px;">Use My Current Location</button>
                        <div class="user-feedback-form-grid" style="grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 10px;">
...
POST['gps_map_url'] ?? '');
    ...
    // Optional photo upload (user-side sample only): keep DB schema unchanged by appending path to description.
    if (isset(<?php
require dirname(__DIR__) . '/session-auth.php';
require dirname(__DIR__) . '/database.php';
require dirname(__DIR__) . '/config-path.php';
...
FILES['photo']['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_NO_FILE) {
        if ((int) <?php
require dirname(__DIR__) . '/session-auth.php';
require dirname(__DIR__) . '/database.php';
require dirname(__DIR__) . '/config-path.php';
