...
rbac_require_from_matrix('contractor.status.request', ['contractor','admin','super_admin']);
...
<head>
    ...
    <title>Requests - Contractor</title>
...
<header class="nav" id="navbar">
    ...
    <div class="nav-links">
        ...
        <a href="requests.php" class="active"><img src="../assets/images/admin/monitoring.png" class="nav-icon" alt="">Requests</a>
...
<section class="main-content" data-contractor-module data-module="requests" data-csrf="<?php echo htmlspecialchars((string)($csrfToken), ENT_QUOTES, 'UTF-8'); ?>">
    <div class="dash-header"><h1>Requests</h1><p>Submit project requests for review and approval.</p></div>
    <div class="cm-card"><div class="cm-form"><select id="cmProject" class="cm-select"></select><select id="cmRequestType" class="cm-select"><option value="">Request Type</option><option>Inspection</option><option>Extension</option><option>Material Approval</option><option>Milestone Review</option></select><input id="cmAttachment" type="file" class="cm-input" accept=".pdf,.jpg,.jpeg,.png"><textarea id="cmDetails" class="cm-textarea full" placeholder="Request details"></textarea><button id="cmSubmitRequest" class="cm-btn" type="button">Submit Request</button></div><div id="cmFeedback" class="cm-feedback"></div></div>
    <div class="cm-card cm-table-wrap"><table class="cm-table"><thead><tr><th>Project</th><th>Type</th><th>Details</th><th>Status</th><th>Date</th></tr></thead><tbody id="cmRequestsBody"><tr><td colspan="5">Loading...</td></tr></tbody></table></div>
</section>
